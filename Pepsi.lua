				...
				if getsynasset then
					...
					elseif #image > 14 and string.sub(image, 1, 14) == "synasseturl://" then
						local x, e = pcall(function()
							...
							local fold = isfolder("./Maru Lib")
							if not fold then
								makefolder("./Maru Lib")
							end
							fold = isfolder("./Maru Lib/Themes")
							if not fold then
								makefolder("./Maru Lib/Themes")
							end
							fold = isfolder("./Maru Lib/Themes/SynapseAssetsCache")
							if not fold then
								makefolder("./Maru Lib Themes/SynapseAssetsCache")
							end
							if not fold or not isfile("./Maru Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat") then
								local res = game:HttpGet(string.sub(image, 15))
								if res ~= nil then
									writefile("./Maru Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat", res)
								end
							end
							return getsynasset(readfile("./Maru Lib/Themes/SynapseAssetsCache/" .. codename .. ".dat"))
						end)
						if x and e ~= nil then
							return e
						end
...
function library:CreateWindow(options, ...)
	...
	if windowName and #windowName > 0 and library.WorkspaceName == "Maru Lib" then
		library.WorkspaceName = convertfilename(windowName, "Maru Lib")
	end
	...
	function windowFunctions:CreateTab(options, ...)
		...
		library.signals[1 + #library.signals] = newTab.MouseButton1Click:Connect(gotoo)
		if windowFunctions.tabCount == 1 then
		...
		function tabFunctions:CreateSection(options, ...)
			...
			if isfolder and makefolder and listfiles and readfile and writefile then
				function sectionFunctions:AddPersistence(options, ...)
					...
					local realDropdownHolderList = Instance_new("UIListLayout")
					local dropdownEnabled = false
					if not isfolder("./Maru Lib") then
						makefolder("./Maru Lib")
					end
					local common_string = "./Maru Lib/" .. tostring(custom_workspace or library.WorkspaceName)
					local function resolvelist(nofold)
						if custom_workspace ~= options.Workspace then
							custom_workspace = options.Workspace
							common_string = "./Maru Lib/" .. tostring(custom_workspace or library.WorkspaceName)
						end
